...
**Sequential Memory:** Unlike timer-based systems that "guess" when a train has passed, this system uses an **S-R Flip-Flop** to reliably "remember" that a train is in the crossing even after it passes the first sensor.
...
**The Problem:** In a simple system, once a train passes the first sensor, the signal drops to $0$.Without memory, the gate would immediately open while the train is still on the tracks.
